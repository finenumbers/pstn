...
1. **Актуальность** — в production ежедневно в 12:00 MSK запускается cron-импорт. Если файлы не изменились (SHA256 совпадает с сохранённым хешем), job завершается со статусом `skipped` без изменения production.
...
flowchart TD
  ...
  swap --> snap[saving_diff_snapshot optional]
...
| `saving_diff_snapshot` | Запись snapshot (если есть сегменты) | `dataset_snapshots`, `number_range_diffs` | «Сохранение расхождений…» |
...
3. Каждый сегмент классифицируется:
   ...
   - **changed** — границы совпадают, но отличается metadata (оператор, регион, ИНН и т.д.) или сдвинуты границы диапазона.
4. Соседние сегменты одного типа **сливаются** (`mergeAdjacentSegments`).
5. Для `changed` в поля `prevRangeStart`, `prevRangeEnd`, `prevOperator` и др. записываются значения **сегмента** из старого датасета (не всего parent range).

### Пример: усечение диапазона

Было: `383 3990000–3999999`. Стало: `383 3990000–3998888`.

| changeType | Диапазон | Цвет в UI |
|------------|----------|-----------|
| changed | 383 3990000–3998888 | Жёлтый |
...
Unlogged temp-таблица — копия `number_ranges` на момент начала import. Существует только во время running import.
...
Фильтры (включая Coverage AND), KPI, facets и экспорт XLSX работают в diff mode. В XLSX добавляется колонка «Тип изменения».
...
- Расписание: **`0 12 * * *`** (ежедневно 12:00 MSK)
