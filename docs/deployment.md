...
# version/revision — только при HEALTH_VERBOSE=1 или NODE_ENV !== production
```
...
В production ответ **минимальный** (без `version`). Для проверки деплоя задайте `HEALTH_VERBOSE=1` в env контейнера app и повторите запрос — появятся `version`, `revision`, `uptimeSec`. Альтернатива: тег образа GHCR / [GitHub Release](https://github.com/finenumbers/pstn/releases). UI **не показывает** номер версии.

### Шаг 3 — Обновление

Portainer → Stacks → `pstn` → **Pull and redeploy** — подтягивает compose из Git и образ `ghcr.io/finenumbers/pstn:latest` с GHCR.

В [`docker-compose.portainer.yml`](../docker-compose.portainer.yml) для `app` задано **`pull_policy: always`** — Docker принудительно тянет `:latest` при каждом redeploy (Compose v2.17+). Зелёная галочка у stack **не** означает, что pull прошёл — сверяйте `HEALTH_VERBOSE=1` + `/api/health`, тег GHCR или GitHub Release.
...
**In-app** (включено с v0.3.19): middleware на `/api/import`, `/api/export/ranges`, `/api/ranges/facets`, `/api/v1/lookup*`. См. [security.md](security.md#in-app-rate-limiting). Переопределение через `RATE_LIMIT_IMPORT`, `RATE_LIMIT_EXPORT`, `RATE_LIMIT_FACETS`, `RATE_LIMIT_LOOKUP`.

**NPM** (рекомендуется как второй слой):
...
| `HEALTH_VERBOSE` | нет | — | `1` — добавить `version`, `revision`, `uptimeSec` в `/api/health` в production |
| `APP_VERSION` | в образе | `dev` | Semver из CI (`Dockerfile` build-arg) |
| `APP_REVISION` | в образе | `unknown` | Git SHA из CI |
...
| `RATE_LIMIT_LOOKUP` | нет | `120/60000` | In-app rate limit `/api/v1/lookup*` |
...
- `ghcr.io/finenumbers/pstn:latest` — Portainer stack (с `pull_policy: always`)
- `ghcr.io/finenumbers/pstn:<version>` — опционально для pin по semver
- `ghcr.io/finenumbers/pstn:<commit-sha>`

Portainer stack использует **`:latest`**. Обновление: CI + GHCR → **Pull and redeploy** → проверка версии через verbose health или Release.
